...
describe('fetchUsage', () => {
  describe('normal quota data', () => {
    ...
    it('clamps usedPct to 0 when percent_remaining > 100', async () => {
      const body = {
        ...BASE_BODY,
        quota_snapshots: {
          premium_interactions: { ...BASE_BODY.quota_snapshots.premium_interactions, percent_remaining: 110 },
        },
      };
  ...
  describe('noData cases', () => {
    ...
    it('returns unlimited=true when pi.unlimited is set', async () => {
      const body = { copilot_plan: 'enterprise', quota_snapshots: { premium_interactions: { unlimited: true } } };
      fetch.mockResolvedValue(mockRes(200, body));
      const data = await fetchUsage('test-token');
  ...
  describe('malformed response handling', () => {
    ...
    it('returns used=0 when entitlement is 0', async () => {
      const body = {
        ...BASE_BODY,
        quota_snapshots: {
          premium_interactions: { ...BASE_BODY.quota_snapshots.premium_interactions, entitlement: 0 },
        },
      };
      fetch.mockResolvedValue(mockRes(200, body));
      const data = await fetchUsage('token');
    ...
    it('returns unlimited=true when pi.unlimited is set alongside percent_remaining', async () => {
      const body = {
        ...BASE_BODY,
        quota_snapshots: {
          premium_interactions: { ...BASE_BODY.quota_snapshots.premium_interactions, unlimited: true },
        },
      };
      fetch.mockResolvedValue(mockRes(200, body));
      const data = await fetchUsage('token');
